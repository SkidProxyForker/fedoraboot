...
needed_deps="wget python3 unzip zip supermin git debootstrap cpio binwalk pcregrep cgpt mkfs.ext4 mkfs.ext2 fdisk depmod findmnt lz4 pv cryptsetup"
if [ "$(check_deps "$needed_deps")" ]; then
  ...
  if [ -f "/etc/debian_version" ]; then
    ...
    apt-get install supermin libguestfs-tools wget python3 unzip zip debootstrap cpio binwalk pcregrep cgpt kmod pv lz4 cryptsetup -y
  fi
